...
function baiduOCR(img) {
  let url, token, opt, result;
  let getRes = (url, data) =>
    data ? http.post(url, data).body.json() : http.get(url).body.json();
  ...
  try {
    ...
    if (!(token = getRes(url)["access_token"])) {
    ...
    if (getRes(url, opt)["words_result"][0]) {
      return getRes(url, opt)["words_result"][0].words;
    } else {
      return "";
    }
  } catch (err) {
